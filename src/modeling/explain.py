...
def feature_importance_table(model, feature_names) -> pd.DataFrame:
    ...
    df_importance = pd.DataFrame({"feature": feature_names[indices],"importance": importances[indices]})
...
def grouped_feature_importance(df_importance: pd.DataFrame) -> pd.DataFrame:
    return df_importance.groupby("feature_group", as_index=False)["importance"].mean().sort_values("importance", ascending=False)